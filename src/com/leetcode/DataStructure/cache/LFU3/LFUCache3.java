...
class LFUCache3 {
    // 存储缓存的内容，Node中除了value值外，还有key、freq、
    // 所在doublyLinkedList、所在doublyLinkedList中的postNode、
    ...
    public void put(int key, int value) {
        ...
        if (node != null) {
            ...
            // 若key不存在
            if (size == capacity) {
                // 如果缓存满了，删除lastLinkedList.pre这个链表（即表示最小频次的链表）中的tail.pre
                // 这个Node（即最小频次链表中最先访问的Node），如果该链表中的元素删空了，则删掉该链表。
            ...
            // cache中put新Key-Node对儿，并将新node加入表示freq为1的DoublyLinkedList中，
            // 若不存在freq为1的DoublyLinkedList则新建。
